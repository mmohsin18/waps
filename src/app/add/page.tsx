...
/** ================== CONFIG: seed/system owner & board ================== */
...
function guessFavicon(origin: string) {
  // Use <img>, not next/image, to avoid remote config hassles
...
/** ================== Page ================== */
export default function AddWapPage() {
  /** --- URL input --- */
  ...
  // If we know the homepage, check DB for it
  ...
  // Prevent duplicate scans on the same URL
  const scannedKeyRef = useRef<string | null>(null)

  /** --- React to normalization & existing doc --- */
  useEffect(() => {
    ...
    // Loading phase for 'existing' query
    ...
    if (existing) {
      ...
      scannedKeyRef.current = null // reset
      return
    }

    // NOT FOUND: scan once per canonical URL
    const key = normalized.canonicalUrl
    if (scannedKeyRef.current === key) {
      // Already scanned and populated form; nothing else to do
      setStatus('new')
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      // 1) Make sure the public board exists (or create it)
      const board =
        seedBoard ??
      ...
      // 3) Add to that board — use board.ownerKey (not the constant)
      await addToBoard({
